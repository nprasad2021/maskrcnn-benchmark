...
def writePickle(outputFile, data):
    p = pickle.Pickler(open(outputFile,"wb"), protocol=2)
    p.fast = True
    p.dump(data)
...
def prPlotCurves(precisionVec, recallVec):
    aucVal = metrics.auc(recallVec, precisionVec)
    plt.plot(recallVec, precisionVec, color='darkorange', lw=2, label='Overall system (area = {0:0.2f})'.format(aucVal))
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.xlabel('Recall')
...
# ROC curve
def rocPlotCurves(fpRateVec, recallVec):
    aucVal = 1.0 - metrics.auc(recallVec, fpRateVec)
    lw = 2
    plt.plot(fpRateVec, recallVec, color='darkorange', lw=lw) #, label='ROC curve (area = %0.2f)' % roc_auc[2])
    plt.plot([0, 1], [0, 1], color='navy', lw=lw, linestyle='--', label='Overall system (area = {0:0.2f})'.format(aucVal))
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.title('ROC curve')
...
def printMsg(msg, verbosity, msgLevel = 1):
...
def dtPrintElapsedTime(text, dtRefTime, verbosity = 10, msgLevel = 0):
    printMsg('Time elapsed {}: {:.0f} ms'.format(text, dtMilliseconds(dtRefTime)), verbosity, msgLevel)

def dtNow():
    return datetime.datetime.now()

def dtMilliseconds(dtRefTime):
    return (dtNow() - dtRefTime).total_seconds() * 1000
